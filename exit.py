...
def update_exit(face_id, cikis_saati, sure):
    rows = []
    with open(attendance_file, "r") as f:
        reader = csv.reader(f)
        for row in reader:
            if row[0] == face_id and row[3] == "":
                row[3] = cikis_saati
                row[4] = sure
            rows.append(row)
                    ...
                    if len(face_distances) > 0 and matches[np.argmin(face_distances)]:
                        ...
                        with open(attendance_file, "r") as f:
                            ...
                            for row in reader:
                                if row[0] == face_id and row[3] == "":
                                    giris_saati = row[2]
                                    cikis_saati = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                    t1 = datetime.strptime(giris_saati, "%Y-%m-%d %H:%M:%S")
                                    t2 = datetime.strptime(cikis_saati, "%Y-%m-%d %H:%M:%S")
                                    sure = str(t2 - t1)
                                    update_exit(face_id, cikis_saati, sure)
                                    print(f"{name} ({face_id}) saved for exit: {cikis_saati}, Time: {sure}")
                                    break
